...
namespace BlazingTwistConfigToolsTest.config.deserialization {
	...
	public class DeserializerUtils_Test {
		...
		private static IEnumerable<string> TestLines => new[] {
				"-key=123",
				"",
				"--deeperKey:\\4\\5\\6\\",
				"",
		...
		private static List<ConfigNodeVerifier> ExpectedNodes => new List<ConfigNodeVerifier> {
		...
		private static IEnumerable<string> nullCharLines => new[] {
				"-value=~",
				"-value2=\\~",
				"-listValue:",
				"--~",
		...
		private static List<ConfigNodeVerifier> ExpectedNullCharNodes => new List<ConfigNodeVerifier> {
		...
		public void Test_Tokenize_Works() {
			List<ConfigNode> tokens = DeserializerUtils.Tokenize(TestLines).ToList();
			List<ConfigNodeVerifier> verifiers = ExpectedNodes;
		...
		public void Test_Tokenize_SupportsNull() {
			List<ConfigNode> tokens = DeserializerUtils.Tokenize(nullCharLines).ToList();
			List<ConfigNodeVerifier> verifiers = ExpectedNullCharNodes;
			Assert.AreEqual(tokens.Count, verifiers.Count);
			int tokenCount = tokens.Count;
			for (int i = 0; i < tokenCount; i++) {
				verifiers[i].Verify(tokens[i]);
